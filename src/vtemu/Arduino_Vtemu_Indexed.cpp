...
#include "Arduino_Vtemu_Indexed.h"

Arduino_Vtemu_Indexed::Arduino_Vtemu_Indexed(int16_t w, int16_t h, Arduino_G *output, int16_t output_x, int16_t output_y, uint8_t mask_level)
...
Arduino_Vtemu_Indexed::~Arduino_Vtemu_Indexed()
{
  if (_framebuffer)
  {
    free(_framebuffer);
...
bool Arduino_Vtemu_Indexed::begin(int32_t speed)
{
...
void Arduino_Vtemu_Indexed::writePixelPreclipped(int16_t x, int16_t y, uint16_t color)
{
...
void Arduino_Vtemu_Indexed::writeFastVLine(int16_t x, int16_t y,
...
void Arduino_Vtemu_Indexed::writeFastHLine(int16_t x, int16_t y,
...
void Arduino_Vtemu_Indexed::drawIndexedBitmap(
    int16_t x, int16_t y,
...
void Arduino_Vtemu_Indexed::flush()
{
  // Serial.printf("GFX_AVI01: x=%d, y=%d, w=%d, h=%d \n",_output_x, _output_y, _width, _height);
  // Serial.flush();

  _output->drawIndexedBitmap(_output_x, _output_y, _framebuffer, _color_index, _width, _height);
}

uint8_t *Arduino_Vtemu_Indexed::getFramebuffer()
{
  return _framebuffer;
}

uint16_t *Arduino_Vtemu_Indexed::getColorIndex()
{
  return _color_index;
}

void Arduino_Vtemu_Indexed::setDirectUseColorIndex(bool isEnable)
{
  _isDirectUseColorIndex = isEnable;
}

uint8_t Arduino_Vtemu_Indexed::get_color_index(uint16_t color)
{
  color &= _color_mask;
...
uint16_t Arduino_Vtemu_Indexed::get_index_color(uint8_t idx)
{
  return _color_index[idx];
}

void Arduino_Vtemu_Indexed::raise_mask_level()
{
